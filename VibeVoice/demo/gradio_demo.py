...
class VibeVoiceDemo:
    ...
    def _save_audio_file(self, audio: np.ndarray, sample_rate: int) -> str:
        output_dir = os.environ.get("VIBEVOICE_OUTPUTS", os.path.expanduser("~/vibevoice_mac/outputs"))
        Path(output_dir).mkdir(parents=True, exist_ok=True)
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        out_path = os.path.join(output_dir, f"vibevoice_{ts}.wav")
    ...
    def generate_podcast_streaming(self, 
        ...
        try:
            ...
            if has_received_chunks and not has_yielded_audio and all_audio_chunks:
                ...
                try:
                    out_path = self._save_audio_file(complete_audio, sample_rate)
                    final_log += f"\n💾 Saved to: {out_path}"
                except Exception as e:
                    final_log += f"\n⚠️ Failed to save audio: {e}"
