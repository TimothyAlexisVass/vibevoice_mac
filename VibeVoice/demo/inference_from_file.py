    ...
    while True:
        try:
            ...
            for idx, ch in enumerate(chunks, 1):
                ...
                t0 = time.time()
                audio = try_generate_once(model, processor, ch, voice_paths, cfg_scale, device_map, max_length_times)
                dt = time.time() - t0
                if audio is None:
...
def parse_args():
    ...
    parser.add_argument("--max_length_times", type=int, default=2, help="Max generation length multiplier vs input length")
